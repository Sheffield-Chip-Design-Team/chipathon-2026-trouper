...
set asic_rtl   [file normalize "[file dirname [info script]]/../../rtl-test"]
...
# MicroBlaze MCS-style automation (creates LMB BRAM, debug, etc.)
...
# Processor system reset for AXI bus clock (100 MHz)
...
connect_bd_net [get_bd_ports ext_resetn]          [get_bd_pins rst_100m/ext_reset_in]

# Processor system reset for DSP clock (32 MHz)
...
# MicroBlaze runs on 100 MHz clk_out1
...
# --- AXI Clock Converter (100 MHz AXI → 32 MHz DSP peripheral) ---------------
set cc [create_bd_cell -type ip -vlnv xilinx.com:ip:axi_clock_converter:2.1 axi_cc_32m]
connect_bd_net [get_bd_pins clk_wiz_0/clk_out1] [get_bd_pins axi_cc_32m/s_axi_aclk]
connect_bd_net [get_bd_pins rst_100m/interconnect_aresetn] \
               [get_bd_pins axi_cc_32m/s_axi_aresetn]
connect_bd_net [get_bd_pins clk_wiz_0/clk_out2] [get_bd_pins axi_cc_32m/m_axi_aclk]
connect_bd_net [get_bd_pins rst_32m/interconnect_aresetn] \
               [get_bd_pins axi_cc_32m/m_axi_aresetn]

# --- axi_dsp_ctrl (our custom peripheral, 32 MHz) ----------------------------
...
# Connect AXI clock converter M-side to axi_dsp_ctrl
connect_bd_intf_net [get_bd_intf_pins axi_cc_32m/M_AXI] \
                    [get_bd_intf_pins axi_dsp_ctrl_0/s_axi]

# Route axi_cc_32m into the AXI SmartConnect that serves the MicroBlaze peripherals
# (connect at the SmartConnect slave port — Vivado BD automation handles numbering)
apply_bd_automation -rule xilinx.com:bd_rule:axi4 \
    -config { Clk_master "/clk_wiz_0/clk_out1 (100 MHz)" \
              Clk_slave  "/clk_wiz_0/clk_out1 (100 MHz)" \
              Master "/microblaze_0 (Periph)" intc_ip "Auto" master_apm "0" } \
    [get_bd_intf_pins axi_cc_32m/S_AXI]
