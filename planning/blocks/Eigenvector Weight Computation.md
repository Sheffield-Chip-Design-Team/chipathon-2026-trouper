...
$$Z = \begin{pmatrix} Z_{00} & Z_{01} & Z_{02} & Z_{03} \\ \overline{Z_{01}} & Z_{11} & Z_{12} & Z_{13} \\ \overline{Z_{02}} & \overline{Z_{12}} & Z_{22} & Z_{23} \\ \overline{Z_{03}} & \overline{Z_{13}} & \overline{Z_{23}} & Z_{33} \end{pmatrix}$$
...
flowchart TD
    ...
    C -- No --> D[Read 6 off-diagonal Z_kl pairs\n int32 I+Q from 0x70–0x8F, 0xD4–0xDB, 0xE0–0xE7]
    D --> E[Read 4 diagonal ZDIAG_k\n uint16 bits 31:16 from 0xE8–0xEF]
    E --> F[Find max_abs across all entries\n diagonal compared at ZDIAG_k × 2^16 scale]
    F --> G{max_abs == 0?}
    G -- Yes --> Z
    ...
    Q --> R[Write W shadow bank\n 0x90–0x9F, 4 complex int16 pairs]
...
### Off-diagonal Z_kl pairs — 6 × complex int32
...
Each is 4 bytes, MSB-first. See `asic_regs.h` for byte-level defines.
...
The full 32-bit equivalent scale is `ZDIAG_k << 16`. Treat them at this scale
when comparing magnitude with the off-diagonal int32 values.

### Accumulation count

`N_ACC` at `0x61`–`0x62` (uint16, big-endian). Skip weight computation if
`N_ACC == 0`.
...
Find the maximum absolute value across all matrix entries, treating the
diagonal at its full int32 scale:
...
### Step 3 — Convert to Q1.15 weights

The MRC weight is the conjugate of the principal eigenvector, normalised to
fill the Q1.15 range:

$$\mathbf{w} = \frac{\overline{\mathbf{v}^{(8)}}}{\|\mathbf{v}^{(8)}\|_\infty}$$

In fixed-point, with $v_{\max} = \|\mathbf{v}^{(8)}\|_\infty$:

$$W_{k,\text{re}} = \left\lfloor\frac{\text{Re}(v_k) \times 32767}{v_{\max}}\right\rfloor, \qquad W_{k,\text{im}} = \left\lfloor\frac{-\,\text{Im}(v_k) \times 32767}{v_{\max}}\right\rfloor$$

Both products fit in int32: $v_k \leq 4096$ and $4096 \times 32767 = 134\text{M} < 2^{31}$.
...
int16 written as two bytes, MSB-first.
...
`0x35`). Write `0x01`, then the hardware self-clears.
...
1. On `IRQ_TRAINING_DONE` with non-zero `N_ACC`, firmware writes all 8 weight
   bytes and pulses `W_COMMIT` within the SF5 timing budget.
...
3. If `N_ACC == 0`, no weight write or commit occurs.
4. The implementation fits within the 4 KiB SRAM budget alongside the rest of
