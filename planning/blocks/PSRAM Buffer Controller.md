...
2. **Margin (armed at `training_done`):** a countdown of `REPLAY_DELAY_SAMPLES` (regs 0x77/0x78, reset 1500 ≈ 3 ms at 500 kS/s) captured samples begins. This is the firmware weight-computation window: the direct 16 MHz rv32emc `IRQ_GROUPER`→production-8-iteration-eigenvector→`W_COMMIT` measurement is 2.145062 ms, leaving 854.938 µs. The combined test asserts this stays below 3 ms; see `planning/blocks/Eigenvector Weight Computation.md` for method. Write-gated `!packet_active`.
