...
Trouper is a standalone hardened DSP macro. In the current top-level RTL it exposes a simple external byte-oriented configuration/status interface rather than embedding PicoRV32 or an on-chip AHB-Lite master/slave fabric. Weight computation is performed by an external board host/controller. Grouper may be used as a development prototype but is **not taped out alongside Trouper** and is not a deployed-system dependency. Trouper SHALL also operate in bypass mode when no weight commit is received or the external control plane is inactive.
...
| TRPR-SYS-003 | C | P | Trouper's digital core operates from a single external 32 MHz clock (IQ_CLK) driving one clock net — no divided or generated clocks. **Frozen board contract:** the TCXO fanout drives all four SX1257 XTB inputs; SX1257_1 `CLK_OUT` drives both IQ_CLK and SX1302 Radio-A clock input on one controlled fanout, while SX1257_2–4 `CLK_OUT` are NC. The **one** clock-domain crossing in the design is the host SPI interface: `HOST_CS`/`SPI_SCK`/`SPI_MOSI` are asynchronous (RPi-driven, ≤2 MHz), resolved by `spi_slave`'s `SPI_SCK`-domain serial engine plus a toggle/mailbox synchroniser on writes and the TRPR-SPS-012 firmware contract on volatile reads (TRPR-SPS-005, Open Risk #38); there are no other CDCs or synchronisers in the core. `reg_bank` updates only on a `ce_16m` clock-enable (honest MCP=2); `packet_ctrl_fsm`, `dc_removal` and `training_acc` are full-rate IQ_CLK flops advanced only by their valid strobes; paced datapath TDM engines carry RTL hold counters (honest MCP=3). The P&R SDC declares only `create_clock -period 31.25` on IQ_CLK plus scoped multicycle constraints matching the RTL pacing; the signoff SDC additionally declares the asynchronous 2 MHz `SPI_SCK` (TRPR-PHY-014). | I |
...
| TRPR-SYS-016 | C | P | **CE-gated / strobe-paced control plane** — only `reg_bank` (incl. interrupt aggregation) is `ce_16m`-gated: it updates on the clock-enable (62.5 ns effective budget, honest MCP=2). `packet_ctrl_fsm`, `dc_removal` and `training_acc` are full-rate IQ_CLK flops whose state advances only on their valid strobes (`iq_tick`/`dcr_valid` etc.); their paced arcs are covered by scoped MCP where the RTL guarantees the claimed stability (Open Risks #39/#40). `spi_slave` is **not** CE-gated: its IQ_CLK-domain half (the toggle/mailbox synchroniser and `reg_we`/`reg_re` generation) runs every IQ_CLK edge, and its serial half runs in the asynchronous `SPI_SCK` domain — the single intentional clock-domain crossing in the design, handled by the synchroniser on writes and by the TRPR-SPS-012 firmware contract on volatile reads (Open Risk #38). All internal handshakes are single-clock and CE- or strobe-aligned by construction. Weight generation is not an RTL block in Trouper; it is performed entirely by an external board host/controller (see §4.6). | I |
...
Single core clock: the external 32 MHz `IQ_CLK` drives every core flop, with no divided or generated clock net. The **one** clock-domain crossing in the whole design is the host SPI interface — the asynchronous, RPi-driven `SPI_SCK` (≤2 MHz) — which `spi_slave` resolves with an `SPI_SCK`-domain serial engine and a toggle/mailbox synchroniser into `IQ_CLK` (TRPR-SPS-005/012, Open Risk #38). No other CDC or synchroniser exists in the core. The timing tiers below are *constraint* tiers on the `IQ_CLK` domain, not separate clock domains:

| Tier | Mechanism | Effective budget | Blocks |
|---|---|---|---|
| Full-rate | single-cycle | 31.25 ns | decimator CIC integrators, `sd_remod`, `psram_buf_ctrl`, `spi_slave` IQ_CLK-domain synchroniser/mailbox |
...
| TRPR-WGN-002 | C | I | Trouper SHALL expose all 6 off-diagonal Z_kl pairs and 4 diagonal Z_kk values in the register bank (see TRPR-TAC-004, TRPR-TAC-005) for firmware to read after `training_done`. The Z/Zdiag banks (`0x40`–`0x6F`) are frozen between window close and the next arm, so a single burst read is coherent **once `TRAINING_DONE` has been confirm-read set** per TRPR-SPS-012 — firmware SHALL NOT read them mid-window. | T |
...
| TRPR-PCF-007 | C | F | A payload-phase timeout SHALL be enforced: if the FSM does not leave `PAYLOAD_ACTIVE` within `PKT_TIMEOUT_SYMS` (0x0B) LoRa symbols of entering it, it SHALL force a return to IDLE and assert `PACKET_DONE` IRQ. The acquisition (`PREAMBLE_ACQ`) and weight-pending (`W_PENDING`) phases are bounded independently by their own deadlines derived from `TACC_WINDOW_SYMS`; `PKT_TIMEOUT_SYMS` does not shorten them. Worst-case `packet_active` duration is the sum of the acquisition, weight-pending, and `PKT_TIMEOUT_SYMS` payload windows — finite for every legal register value, but `PKT_TIMEOUT_SYMS` is not a global watchdog and cannot abort a bad acquisition early (Open Risks #64, resolved 2026-09-04 by this clarification; no RTL change). | T |
...
Register-mediated PSRAM reads over host SPI (no Grouper required) for bring-up and capture inspection. Note the `PSRAM_DBG_DATA` (`0x76`) and `PSRAM_DBG_WDATA` (`0x79`) burst auto-increment exceptions in TRPR-SPS-010.
...
| TRPR-SPS-005 | C | HW | `HOST_CS`, `SPI_SCK`, and `SPI_MOSI` are asynchronous to the 32 MHz core clock — the host SPI interface is the one intentional clock-domain crossing in the design. As implemented, `spi_slave` runs its serial engine in the `SPI_SCK` domain and crosses completed register events into the core on a persistent-toggle + bundled-data mailbox synchroniser; the register *read* tap is combinational and its coherency is handled by firmware per TRPR-SPS-012 (Open Risk #38). | I |
...
| TRPR-SPS-010 | H | F | **Burst access:** if `HOST_CS` remains asserted after the first data byte, each additional data byte SHALL access the next consecutive register address (auto-increment, wrapping modulo 128). Exception: the PSRAM debug data ports `PSRAM_DBG_DATA` (`0x76`, read) and `PSRAM_DBG_WDATA` (`0x79`, write) SHALL NOT auto-increment — repeated data bytes re-access the same port (`0x76` re-reads the fetched window byte-by-byte; `0x79` pushes successive bytes into the write shadow). RTL: `spi_slave.v` `NO_INC_RD_ADDR`/`NO_INC_WR_ADDR`. | T |
...
| TRPR-SPS-012 | C | P | **Volatile-read firmware contract (Open Risk #38, "Route 2" — accepted probabilistic mitigation for this revision, not a hardware coherency guarantee).** `spi_slave` **directly samples** the *combinational* `reg_bank` read tap on the falling `SPI_SCK` edge (TRPR-SPS-009) with no synchroniser on the read path, and `SPI_SCK` is asynchronous to the 32 MHz core (TRPR-SPS-005). A register field a hardware event can change between two SCK edges can therefore return a torn byte on a single read, and any bit can go momentarily metastable. The rules below **reduce the probability of accepting a bad byte; they do not make the crossing coherent or remove metastability** — residual CDC risk remains under Open Risk #38. Host firmware SHALL: (a) make no control decision on a single sample of a volatile field — require **two independent command-plus-data transactions to the same address, `HOST_CS` deasserted between them, returning the identical byte** (burst mode auto-increments the address, so two consecutive bytes of one CS-low transaction are *different registers* and do not count), with a bounded retry cap (recommended 4) after which the value is a bus error, not data; (b) read multi-byte volatile values (e.g. `N_ACC` mid-window) in full in each transaction and use them only when the two complete copies are byte-identical; (c) read the frozen result banks — `Z_kl`/`ZDIAG_k` (`0x40`–`0x6F`) and the final `N_ACC` (`0x21`–`0x23`) only after `PACKET_STATUS.TRAINING_DONE` 0x1C[4] (or `IRQ_STATUS.NOISE_READY`) has been confirm-read set per (a), completing the burst before the next `sc_lock`/`TACC_NOISE_TRIG` re-arms the window; `SC_FIRST_HIT` (`0x28`–`0x2B`) / `SC_LOCK_SNAP` (`0x2C`–`0x2F`) after the corresponding lock/first-hit — within that gap a single burst read *is* coherent (frozen data, stable address, no CDC hazard). `SC_STAT` (`0x24`–`0x25`) and `SC_DBG_FLAGS` (`0x26`) are **volatile**, not frozen (re-assigned from live detector state; firmware SHALL NOT assume they freeze after lock). Mid-window `N_ACC` is **not recoverable** by confirm-read (it increments faster than a read completes) — treat it as a diagnostic progress indicator only. (d) apply the volatile rule to any burst that spans a mix of static and volatile addresses; (e) prefer polling for a stable level over sampling a transient, and never use a volatile field as the sole input to a correctness-critical decision. The write path is unaffected (persistent toggle + mailbox, Open Risk #15). One read is a full command+data transaction (16 SCK ≈ 8 µs at 2 MHz), so a confirmed volatile field costs **≈ +8 µs** (a second transaction) — tens of µs over a weight-transfer sequence, negligible against the compute budget (TRPR-WGN-004) at every SF. An on-chip core-domain read snapshot that would give a real hardware guarantee ("Route 1") is a post-tapeout option. Authoritative detail and the per-register volatile/static/frozen classification: `planning/Register Map.md` § *Host SPI read coherency — firmware contract*. | A |
...
Trouper has no on-chip SPI master. External board-controller firmware owns SX1257 LNA gain control through the board-level SPI master and uses Trouper's Zdiag registers for power measurement. Grouper is not taped out alongside Trouper.
...
| TRPR-AGC-002 | C | I | The AGC strategy SHALL be "maximum gain before saturation": external board-controller firmware SHALL increase LNA gain unless Zdiag/n_acc exceeds a firmware-held high-water threshold, and decrease gain if it exceeds a firmware-held saturation threshold. **No on-chip `AGC_THR_HI`/`AGC_THR_SAT` comparator registers exist** — both thresholds live entirely in external controller memory; the comparison is software against the Zdiag/n_acc value read per TRPR-AGC-001 (see `planning/Register Map.md` "Former addresses": these register names were never implemented in RTL). One SX1257 LNA gain step per packet. All four antennas are controlled independently. Grouper is not taped out alongside Trouper and is not a required AGC component. | T |
| TRPR-AGC-003 | — | — | **REMOVED.** `RX_GAIN_SHADOW_0..3`/`RX_GAIN_ACTIVE_0..3`/`RX_GAIN_CTRL` at `0x10`–`0x18` were removed (2026-07-28): Trouper has no SX1257 SPI/control outputs (see this section's header note), so an on-chip shadow→active gain-commit latch had no hardware consumer and only mirrored software-written values. Gain programming is entirely external through the board-level SPI master; Grouper is not taped out alongside Trouper. See `planning/Register Map.md` "Removed registers". | — |
...
| TRPR-AGC-005 | C | I | External board-controller firmware SHALL maintain a per-antenna noise-floor EMA: σ²_ema[k] ← (1−α)·σ²_ema[k] + α·(Zdiag[k]/n_acc). A single per-packet noise sample is not sufficient for stable weight quality. This EMA supplies the optional noise-weighted MRC mode (TRPR-WGN-012), which scales conventional MRC weights by `1/σ²_ema[k]`. | T |
...
| TRPR-INT-006 | H | I | The digital core runs on the single 32 MHz clock; the control-plane peripheral `reg_bank` is clock-enable-gated to an effective 16 MHz update rate (`clk_en`, honest MCP=2 — no separate `CLK_16M` net or generated clock exists). `spi_slave` is not CE-gated (`packet_ctrl_fsm`, `dc_removal`, `training_acc` are strobe-paced full-rate flops). The **only** clock-domain crossing is the host SPI interface: `spi_slave` runs its serial engine in the asynchronous `SPI_SCK` domain and crosses completed register events into `IQ_CLK` on a persistent-toggle + bundled-data mailbox (2-/3-FF synchroniser). Config-register and fixed-ID reads are stable across the crossing; volatile 32 MHz status read back over MISO is mitigated by the TRPR-SPS-012 firmware contract (probabilistic, residual risk under Open Risk #38). No other synchronisers exist in the core. (Amended 2026-09-01: Grouper register bus removed. Amended 2026-09-05: CDC description corrected to match `spi_slave.v` / `packet_ctrl_fsm.v`.) | I |
| TRPR-INT-007 | H | F | External board-controller firmware and host SPI SHALL use the same Trouper register map as defined in `planning/Register Map.md`. No separate Trouper-only firmware register space exists. | I |
...
| ID | Pri | Type | Requirement | Verif |
|---|---|---|---|---|
| TRPR-INT-010 | C | F | When the external weight-control path is inactive or no `W_COMMIT` is received, Trouper SHALL operate in bypass mode: the combiner routes the lowest-enabled antenna to the re-modulator output without MRC weighting. | T |
