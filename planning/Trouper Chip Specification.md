...
| TRPR-REG-006 | H | F | Write-1-Pulse (W1P) bits (`TACC_NOISE_TRIG` 0x1F[0], `WGT_CTRL.W_COMMIT` 0x1E[0], `RX_GAIN_CTRL.RX_GAIN_COMMIT` 0x18[0], `PSRAM_CTRL.PSRAM_CLR_ERR` 0x70[1], `PSRAM_DBG_CTRL.RD_TRIG` 0x75[0]) SHALL self-clear on the cycle after assertion. (Naming normalised 2026-07-26, audit item 25: three of the five were listed by bit name where the others used `REGISTER.BIT`, which read as if `RX_GAIN_COMMIT` and `PSRAM_CLR_ERR` were registers in their own right — they are bit [0] of `RX_GAIN_CTRL` and bit [1] of `PSRAM_CTRL`. All five are bits, not registers; only `TACC_NOISE_TRIG` occupies an address of its own.) | T |
