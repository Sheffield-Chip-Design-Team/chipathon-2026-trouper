...
6. **PSR-001's real gap (tPU wait) is known and tracked** (Open Risks #55, `[[project_startup_delay_risks]]` memory) — flagged here for cross-reference, not a new finding.
...
| TRPR-PCF-007 | Payload-phase timeout via `PKT_TIMEOUT_SYMS` (0x0B) forces IDLE, asserts `PACKET_DONE` IRQ | T | `tb_trouper_two_packet.v` (job 3203), `cocotb/tests/test_w_missed_packet.py` (job 3305), `cocotb/pkt_timeout_states/` (job 5474) | ✅ Spec address fixed 2026-07-05 (was `0x16`). The `PACKET_DONE` IRQ bit closed 2026-07-06: `test_w_missed_packet.py` writes `PKT_TIMEOUT_SYMS=20` pre-lock and polls `IRQ_STATUS[3]` firing on the timeout→IDLE transition, then confirms `PACKET_STATUS` reads idle and the receiver re-locks. **Scope clarified 2026-09-04 (Open Risks #64, doc-only):** `PKT_TIMEOUT_SYMS` bounds the payload phase only; `PREAMBLE_ACQ`/`W_PENDING` have independent `TACC_WINDOW_SYMS`-derived deadlines. `packet_active` is finite for every legal value but this register is not a global watchdog and cannot abort a bad acquisition early. |
...
| TRPR-MRC-004 | Live W bank is consumed after `W_COMMIT`/`W_VALID`; writes are rejected while valid | T | `test_weight_gen_spi_flow.py` | ✅ Spec corrected 2026-07-26: `mrc_combiner.v` reads `rb_w_shadow` directly; no separate `W_ACTIVE` latch or safe-switch promotion exists. The RTL write-lock prevents a mid-packet W-bank rewrite while `W_VALID` is high. ⚠️ **Coverage gap (2026-09):** the sticky `WGT_CTRL.W_WR_REJECTED` bit (`0x1E[5]`) that the revised spec now mandates on a rejected locked-bank write is not read back by this test — only the write-lock's *effect* (live vector unchanged) is checked, not the flag. See MRC-011 and open item 6. |
...
| TRPR-MRC-011 | `WGT_CTRL` (0x1E) exposes `W_COMMIT`/`W_VALID`/`W_PENDING`/`W_MISSED_PACKET`/`W_COMMIT_LATE`/`W_WR_REJECTED` | I | `tb_trouper_spi.v` (`W_COMMIT` W1P mechanics), `cocotb/tests/test_w_missed_packet.py` (job 3305) | ⚠️ **Partial.** `W_PENDING` (bit[2]) and `W_MISSED_PACKET` (bit[3]) read over SPI in a live packet 2026-07-06 — including that `W_MISSED` stays readable through IDLE and clears at the next packet start (needed the `W_missed_q` RTL fix, see PCF-005). `W_VALID` (bit[1]) is checked in the 0 case here and implicitly in the 1 case by `test_weight_gen_spi_flow.py`'s commit flow. **Two bits added in the 2026-09 spec revision are not covered:** `W_COMMIT_LATE` (bit[4]) is exercised as behaviour by the TRPR-PSR-004 late-commit path but never asserted as a `0x1E[4]` readback; `W_WR_REJECTED` (bit[5], RO/W1C) has no readback or W1C-clear test at all. See open item 6. |
...
4. **MRC-009 (AGC-enforced input headroom) has zero RTL coverage** and can't have any until AGC firmware itself is tested (Open Risks #8) — cross-referenced, not a new finding. **Spec-side follow-up:** change `TRPR-MRC-009`'s `Verif` column in `Trouper Chip Specification.md` from `T` to `P` — this matrix already carries the correct classification (`P`, firmware/analysis).
...
6. **`WGT_CTRL` sticky bits `W_COMMIT_LATE` (0x1E[4]) and `W_WR_REJECTED` (0x1E[5], RO/W1C) — added to TRPR-MRC-004/011 in the 2026-09 spec revision — are untested at the register level.** `W_COMMIT_LATE` rides along with the TRPR-PSR-004 late-commit path but is never read as `0x1E[4]`; `W_WR_REJECTED` has no test at all. One directed SPI test can cover both: provoke a locked-bank write (expect `W_WR_REJECTED` set), read/W1C-clear it, then land a commit after replay start but before `packet_end` (expect `W_COMMIT_LATE` set).
...
| ID | Requirement (short) | Verif | Test(s) | Status |
|---|---|---|---|---|
| TRPR-RMD-001 | 2× droop-comp interpolator + 4th-order CIFF ΣΔ core, int8 I/Q → 1-bit I/Q (was 3rd-order; core raised 2026-09-04, interpolator added + arithmetic made shift/add-only 2026-09-05) | T | `tb_sd_remod.v`, `tb_remod_decim_const.v`, NFS `test_capture_playback.py` (2026-08-30) | ⚠️ **Cited evidence predates the 2026-09-04/05 topology change (3rd→4th-order core + 2× interpolator) — see open item 5.** In the measured SF7/BW125 NFS run on `lora_20260619_162950_SF7-BW125-Pre8.npy[360000:390000]`, an independent Python FFT brick-wall LPF (250 kHz) and R=64 decimation of 8,192 final 1-bit I/Q clocks yields complex correlation **0.992** with the simultaneously recorded `comb_y` interval (best ±8 output-sample alignment; acceptance threshold >0.70). This is an end-to-end transfer check, not a replacement for the standalone SQNR requirements. |
...
| TRPR-RMD-009 | Input SHALL NOT jump backward in signal time-index (except first-ever lock after `RESETB`); silence→signal and mid-stream weight changes are fine | T | `cocotb/tests/test_replay_delay.py` monotonic-`rd_ptr` check (job 3347) | ✅ Verified via **TRPR-PSR-024** — the continuous-delay replay keeps `rd_ptr` monotonically non-decreasing for the life of a packet, which is the mechanism that satisfies this requirement (`planning/psram-replay-continuous-delay-redesign.md`, implemented 2026-07-12). Traced primarily under PSR-024; row kept here so a grep-based refresh of this section does not drop RMD-009. |
...
5. **This section is stale against the 2026-09-04/05 re-modulator rewrite.** `TRPR-RMD-001` now specifies a *2× droop-compensating interpolator ahead of a 4th-order CIFF ΣΔ core* — the core was raised from 3rd to 4th order on 2026-09-04 to absorb the loop's one-sample excess loop delay, and on 2026-09-05 the interpolator was promoted to close the STF droop at fs/4 with the core arithmetic rewritten shift/add-only (claimed bit-exact vs. the pre-rewrite core; see `planning/sd-remod-interp2x-pnr-investigation-2026-09-05.md` and the `sd_remod.v` header). Every test cited for RMD-001/003/005/006/007 (`tb_sd_remod.v`, `test_remod_backoff.py`, `test_remod_en.py`, `test_capture_playback.py`, all ≤ 2026-08-30) predates that change. No post-rewrite SQNR / dither / transfer evidence is recorded here yet. RMD-001 marked ⚠️ pending a re-run; RMD-003/005/006/007 stay ✅ on the shift/add bit-exactness claim but their evidence must be re-confirmed against the 4th-order + interpolator topology before signoff.
...
| TRPR-REG-002 | ~~`GRP_*` **byte** request/ack bus reachable from SPI bridge and Grouper master, Grouper priority~~ | — | ~~`tb_trouper_grp_arb.v`~~ (bench deleted 2026-09-01) | ⛔ VOID 2026-09-01 — Grouper not taping out; the `GRP_*` bus, AHB endpoint and `IRQ_GROUPER` were removed from `trouper_top.v`, leaving host SPI as the sole register master. Was ✅ |
...
## Bring-Up Sample Source — TRPR-BRU

Deterministic first-silicon stimulus at the combiner input, so `mrc_combiner`
and `sd_remod` can be proven without a working frontend, SC acquisition or
PSRAM replay path. See `planning/foundational-block-bringup-plan.md`.

| ID | Requirement (short) | Verif | Test(s) | Status |
|---|---|---|---|---|
| TRPR-BRU-001 | Reset selects the normal path; `BRINGUP_CTRL`/`AMPL` reset to 0 | T | `test_bringup_src.test_reset_selects_the_normal_path` | ✅ Job 5427 |
| TRPR-BRU-002 | Disabled source cannot perturb the re-modulator input stream | T | `test_bringup_src.test_disabled_source_does_not_perturb_the_path` (3000 cycles bit-identical on `remod_in_*`, armed-then-disarmed vs baseline, identical SPI traffic in both arms) | ✅ Job 5427 |
| TRPR-BRU-003 | Arming gated on `RX_HOLD=1 && !PACKET_ACTIVE`; rejection sets `CFG_WR_REJECTED` | T | `test_bringup_src.test_arming_is_refused_unless_rx_hold`, `test_arming_is_refused_during_a_live_packet`, `test_level_term_holds_if_the_bit_is_forced_during_a_packet` | ✅ Job 5427. Both barriers covered: `reg_bank`'s `cfg_wr_ok = rx_hold && !packet_active` refuses the write, and the `trouper_top` level term holds independently (exercised by forcing the register past the write gate). Note `packet_ctrl_fsm` never sees `rx_hold`, so raising `RX_HOLD` mid-packet does not end the packet |
| TRPR-BRU-004 | Qualified enable drops continuously when `RX_HOLD` is released | T | `test_bringup_src.test_enable_drops_when_rx_hold_is_released` | ✅ Job 5427 |
| TRPR-BRU-005 | Every generator mode emits its specified stream | T, F | `test_bringup_src.test_zero_mode`, `test_dc_mode_polarity`, `test_tone_mode_is_a_quadrature_rotation`, `test_prbs_mode_switches_and_stays_bounded`, `test_prbs_matches_the_golden_lfsr` (x^9+x^5+1 reference, sample for sample); `formal/bringup_src_formal.sv` zero-mode property | ✅ Job 5427 + formal job 5438 |
| TRPR-BRU-006 | Amplitude clamped to ±64 — `sd_remod` NTF is permanently unstable on wrap-around | T, F | `test_bringup_src.test_amplitude_is_clamped`; `formal/bringup_src_formal.sv` group A (proved over all 256 values of `ampl`, incl. `0x80` where an 8-bit magnitude would negate to itself and bypass the clamp) | ✅ Job 5427 + formal PASS job 5438. Load-bearing now that the source drives `sd_remod` directly, ahead of `REMOD_BACKOFF_SHIFT` — nothing downstream can rescue an out-of-range sample |
| TRPR-BRU-007 | Deterministic restart after reset in every mode | T | `test_bringup_src.test_deterministic_restart_after_reset` | ✅ Job 5427 |
| TRPR-BRU-008 | Samples reach `sd_remod`'s input unmodified; combiner is out of the path | T | `test_bringup_src.test_samples_reach_the_remod_input`, `test_combiner_is_out_of_the_path_while_armed`, `test_armed_source_overrides_the_backoff_shift`, `test_armed_source_overrides_psram_silence`, `test_bringup_overrides_the_live_normal_path`, `test_disarm_returns_the_path_without_a_stuck_output` | ✅ Job 5427. Rewritten for the 2026-09-03 move to the re-modulator input; the old four-branch combiner fan-out check no longer applies |
| TRPR-BRU-009 | Area and SS/hold timing cost measured against an identical build | A | Combiner insertion: job 5404 vs 5379. Re-modulator insertion: jobs 5425/5436 | ❌ **Not met at the chosen insertion point — the netlist does not route.** Jobs 5425 and 5436 both fail detailed routing with `DRT-0073 No access point for clkbuf_2_2_0_IQ_CLK_regs/I`, deterministically, so no timing or area figure exists for option (c) yet. Notably the failing netlist is **smaller** than the combiner variant that routed clean (35,436 vs 35,597 at synthesis; 48,900 vs 49,020 at CTS), which disproves `trouper_top.json` `_comment_density`'s framing of this failure class as netlist-size sensitive — the mechanism is placement perturbation around the `IQ_CLK` tree. Probes: `DPL_CELL_PADDING` 3 (job 5439, cancelled, not evaluated); `PL_TARGET_DENSITY_PCT` 64 (job 5440, pending). Superseded reference point — combiner insertion, job 5404 vs 5379: +8,790 µm² (+0.750%), +349 cells, SS WNS −10.130 → −10.916 ns, SS TNS −383.5 → −699.3 (1.82×), DRC/antenna/XOR/LVS all 0 |
| TRPR-BRU-010 | Captured 1-bit output matches the programmed DC/tone after external decimation | T | `test_bringup_src.test_dc_signature_at_the_remod_output` (±1 output density = level/127 for levels 16/32/48, per rail, no filter needed), `test_tone_signature_at_the_remod_output` (R=64 brickwall reconstruction vs `A/127·exp(+jπn/2)`, amplitude **and** rotation direction — a swapped or inverted rail leaves per-rail densities untouched and shows only as the conjugate tone; whole-band SQNR 18.2 dB) | ✅ Job 5427. Tests I and Q independently (DC, per rail) and together (tone direction), as the plan requires |
| TRPR-BRU-011 | Board bring-up procedure documented | D | `planning/Register Map.md` §`0x06`–`0x07` | ✅ Updated 2026-09-03 for the re-modulator insertion point |
| TRPR-BRU-012 | Generator cadence is exactly 500 kS/s (one valid per 64 clocks, no drift) | T, F | `test_bringup_src.test_sample_cadence_is_500ksps`; `formal/bringup_src_formal.sv` group C | ✅ Job 5427 + formal job 5438. `sd_remod`'s NTF is synthesised for OSR=64, so a wrong divider silently invalidates every downstream signature check |
| TRPR-BRU-013 | Debug probe COMB group observes the injected sample | T | `test_bringup_src.test_debug_probe_comb_group_shows_the_injected_sample` | ✅ Job 5427. Consequence of the move: the probe taps `remod_in_i/q`, now downstream of the mux, so an armed source is visible on `DBG0`/`DBG1` as well as the pads |
...
| TRPR-IRQ-003 | `IRQ_OUT` driven by `\|irq_status` (the `IRQ_GROUPER` half was removed 2026-09-01) | T | `cocotb/irq_pins/test_irq_pins.py::test_irq_pins_sticky_clear` (Verilator, 2026-08-29, 1/1 PASS, 2.33 s) | ✅ Both top-level pins are sampled directly: they assert together when `TRAINING_DONE` and `NOISE_READY` become sticky, and are always equal. |
...
| TRPR-INT-002 | ~~Grouper path reaches full register map~~ | — | ~~`tb_trouper_grp_arb.v`~~ (bench deleted) | ⛔ VOID 2026-09-01 — Grouper not taping out; interface removed from `trouper_top.v`; was ✅ |
| TRPR-INT-003 | ~~Arbiter: Grouper priority, SPI-write pending slot, read retry~~ | — | ~~`tb_trouper_grp_arb.v`~~ (bench deleted) | ⛔ VOID 2026-09-01 — Grouper not taping out; interface removed from `trouper_top.v`; SPI is now the only master, so nothing arbitrates; was ✅ |
| TRPR-INT-004 | SPI-only operation clean (unconditional since 2026-09-01; was "Grouper idle/reset →") | T | `cocotb/tests/test_host_only_e2e.py::test_host_spi_only_same_packet_mrc_replay` (Verilator, 2026-08-29, 1/1 PASS, 2.19 s); all other top-level cocotb suites also tie `GRP_*` off in `tb_trouper_cocotb.v` | ✅ **Dedicated standalone proof added 2026-08-29.** Explicitly ties `GRP_ADDR`/`GRP_WDATA`/`GRP_WE`/`GRP_RE` low and rechecks them at every host-SPI transaction boundary. Host SPI alone enables and initialises PSRAM, configures and locks RX, observes `TRAINING_DONE`, reads `ZDIAG`, writes/commits W, and reaches monotonic same-packet MRC replay without `REPLAY_MISSED`. |
| TRPR-INT-005 | ~~IRQ line to Grouper on any status bit~~ | — | `cocotb/irq_pins/test_irq_pins.py::test_irq_pins_sticky_clear` (Verilator, 2026-08-29, 1/1 PASS) | ⛔ VOID 2026-09-01 — Grouper not taping out; interface removed from `trouper_top.v`; was ✅ `IRQ_GROUPER` is directly sampled alongside `IRQ_OUT` through assertion, selective clear, final clear, and post-clear dwell. |
...
| TRPR-INT-010 | Bypass when no `W_COMMIT` arrives ("Grouper inactive" clause void 2026-09-01) | T | `test_w_missed_packet.py`, `test_bypass_e2e.py` (jobs 3304–3310) | ✅ |
| TRPR-INT-011 | Host pre-config over SPI without firmware | T | every cocotb suite's setup sequence is exactly this | ✅ |
| TRPR-INT-012 | Mode-1 = functional single-antenna path ("Grouper-inactive" qualifier void 2026-09-01) | T | `test_bypass_e2e.py` (job 3304, raw stream to remod end-to-end) | ✅ |
