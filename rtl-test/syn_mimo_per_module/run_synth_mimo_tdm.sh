#!/bin/bash
# Hierarchical synthesis of mimo_rx_top with sd_decimator_top (TDM).
# Reports per-module area and compares total vs monolithic baseline.
# Baseline: run from out_combchain/yosys.log (uses sd_decimator_combchain × 4).
set -euo pipefail
...
mkdir -p "$OUT"
LOG=$OUT/run.log
echo "=== mimo_rx_top TDM synth $(date --iso-8601=seconds) on $(hostname) ===" | tee "$LOG"
...
setundef -zero
opt_clean -purge
tee -o $OUT/stat.txt stat -liberty $CORE_LIB -top mimo_rx_top
YEOF
...
if [ -f "$BASELINE_STAT" ]; then
    echo "  monolithic (sd_decimator × 4):" | tee -a "$LOG"
    grep "Chip area for top module" "$BASELINE_STAT" | tee -a "$LOG"
    echo "  TDM (sd_decimator_top × 1):" | tee -a "$LOG"
    grep "Chip area for top module" "$OUT/stat.txt" | tee -a "$LOG"
    python3 -c "
import re
def get_area(path):
