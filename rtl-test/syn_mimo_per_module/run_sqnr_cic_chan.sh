#!/bin/bash
# Verify sd_cic_chan interface cut: wraps sd_cic_chan + inline FIR in
# tb_cic_chan_wrap.v (renamed sd_decimator), runs against tb_sqnr.v.
# Pass: SQNR >= 28 dB AND bit-exact match vs combchain baseline.
set -euo pipefail
RTL=/foss/designs/lora-mimo
OUT=$RTL/rtl-test/syn_mimo_per_module/out_cic_chan
TB_DATA=$RTL/sim/tb_data
mkdir -p "$OUT" "$TB_DATA"
LOG=$OUT/run.log
echo "=== sd_cic_chan SQNR verification $(date --iso-8601=seconds) on $(hostname) ===" | tee "$LOG"
...
for VARIANT in combchain cic_chan_wrap; do
    if [ "$VARIANT" = "combchain" ]; then
        SRCS="$SWAP_CC"
    else
        SRCS="$RTL/rtl-test/tb_cic_chan_wrap.v $RTL/rtl-test/sd_cic_chan.v"
    fi
...
OUT = "/foss/designs/lora-mimo/rtl-test/syn_mimo_per_module/out_cic_chan"
...
for v in ["combchain", "cic_chan_wrap"]:
    r = sqnr(os.path.join(OUT, f"rtl_out_{v}.txt"))
    for ch, (db, amp, n) in r.items():
        flag = "PASS" if db >= 28.0 else "FAIL"
        print(f"  [{v:13s}] {ch}: SQNR {db:5.2f} dB  amp {amp:6.1f}  n={n}  [{flag}]")

print()
cc = np.loadtxt(os.path.join(OUT, "rtl_out_combchain.txt"),    dtype=np.int8).reshape(-1,2)[SKIP:]
ww = np.loadtxt(os.path.join(OUT, "rtl_out_cic_chan_wrap.txt"), dtype=np.int8).reshape(-1,2)[SKIP:]
n = min(len(cc), len(ww))
diffs = np.abs(cc[:n].astype(int) - ww[:n].astype(int))
mis = (diffs > 0).any(axis=1).sum()
print(f"  combchain vs cic_chan_wrap: {n} samples, max_diff={diffs.max()}, mismatches={mis}")
if mis == 0:
    print("  RESULT: BIT-EXACT — sd_cic_chan interface cut is correct")
else:
    print("  RESULT: MISMATCH — investigate latency or logic difference")
    bad = np.where((diffs > 0).any(axis=1))[0][:5]
    for i in bad:
