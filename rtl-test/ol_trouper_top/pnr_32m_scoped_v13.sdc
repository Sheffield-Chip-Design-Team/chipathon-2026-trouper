...
# --- SF/BW config → quasi-static barrel shifts: false_path ----------------------
# rb_sf_cfg / rb_bw_sel / rb_sample_shift are write-locked during a packet (the
# BW-lock is verified in the cocotb test) and change only BETWEEN packets (host
# writes at kHz), so the deep barrel-shift / address cones they feed
# (M = 1<<(SF+shift) in u_sc/u_tacc, del_offset = 8<<(sf+shift) in u_psram) have
# thousands of cycles to settle.  false_path is arc-specific — it only relaxes
# paths THROUGH these config nets, not the live counter/datapath arcs.
set cfg_qstatic [get_nets -hierarchical {rb_sf_cfg[*] rb_bw_sel rb_sample_shift[*]}]
set_false_path -through $cfg_qstatic
