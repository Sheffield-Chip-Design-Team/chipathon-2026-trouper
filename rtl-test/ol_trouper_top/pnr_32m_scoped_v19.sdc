# pnr_32m_scoped_v19.sdc
# v19 = v18 + u_pcfsm added to the scoped-MCP=3 wildcard.
#   The training-window-control feature added rb_tacc_window_syms (reg_bank
#   0x27, firmware-writable, quasi-static like rb_sf_cfg) driving a variable
#   barrel-shift + 32-bit add in packet_ctrl_fsm (acq_timeout_next /
#   wpend_timeout_next), registered into acq_timeout_q/wpend_timeout_q. This
#   surfaced as the new SS worst path (rb_tacc_window_syms[1] -> ~40 gates ->
#   packet_ctrl_fsm reg, WNS -23.08ns, job 3182) because u_pcfsm was not in
#   the paced-block wildcard. Same broad `-through {hier.*}` form already
#   proven safe for u_dec/u_sc/u_tacc/u_comb (see v9 fix note below) — not the
#   narrow specific-net form that caused the v15 silent no-op.
#
...
# --- Scoped multicycle: paced DSP blocks + packet_ctrl_fsm get 3 cycles -------
# Net names retain hierarchy ('.' separator) after flatten; cell names do not.
# u_pcfsm added in v19 (see header) for the tacc_window_syms-driven timeout adder.
set paced_nets [get_nets -hierarchical {u_dec.* u_sc.* u_tacc.* u_comb.* u_pcfsm.*}]
