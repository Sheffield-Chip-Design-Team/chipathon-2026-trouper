...
#   single-cycle.
# CONTROL ONLY (2026-08-16): sc_clear exception RESTORED, byte-identical to
# the pre-v26 form. This file exists solely to isolate the exception's true
# cost on the signoff configuration -- job 4376 showed packet_done_pulse paths
# violating at up to -5.35 ns once withdrawn, contradicting a +3.94 ns MET
# measurement that had been taken on job 3738's 9ns-margin netlist (not
# representative of config_current_signoff). Do NOT promote this to canonical:
# the exception is unsound (1-cycle pulse, capture at t+1). See Open Risks #43.
set sc_clr_srcs [get_nets {packet_done_pulse}]
set sc_clr_regs [get_cells -of_objects \
    [get_nets -hierarchical {u_sc.hit_count[*] \
...
set mcp_audit_groups {
    {sc_clear                 3 2 sc_clr_srcs          sc_clr_regs}
