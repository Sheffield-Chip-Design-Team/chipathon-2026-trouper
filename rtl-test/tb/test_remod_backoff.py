...
from test_trouper_top import CLK_NS, spi_read, spi_write, spi_burst_write, sdm_driver
...
async def _check_shift_wiring(dut, shift, n=60):
    ...
    checked = 0
    for _ in range(n * 100):   # comb_y_valid pulses once per 64 IQ_CLK cycles
        ...
        if yv.is_resolvable and int(yv):
            ...
            if checked >= n:
                break
    assert checked >= n // 2, f"shift={shift}: only {checked} comb_y_valid samples observed"
