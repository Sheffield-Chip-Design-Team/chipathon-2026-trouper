"""
test_trouper_top_hb.py — cocotb integration tests for trouper_top_hb.

Sweeps SF7–SF12 × BW 250 kHz / 125 kHz using the half-band decimator chain:
  32 MS/s → CIC-3/R=16 → HB1/2 → HB2/2 → 500 kS/s int8 IQ

Key differences from test_trouper_top.py (CIC-only R=128, 250 kS/s):
  - clk_per_iq = 64 (iq_valid every 64 clocks; HB chain fixed R=64)
  - sample_shift = 1 (250 kHz BW, 2× oversampled) or 2 (125 kHz BW, 4× oversampled)
...
async def sdm_driver(dut, sf, bw_khz):
    ...
    frequency the dc_removal_hb highpass (corner ≈ 1/32) attenuates by <3%
    ...
    SF7/BW250 (A²×256/1024 = 240 >> 91 ✓) and accumulator headroom holds
    to SF12/BW125 (A²×16384/1024 = 15376 < 16384 = 2^14 ✓ for 24-bit acc).

    clk_per_iq = 64: sd_decimator_hb_tdm has fixed R=64 (CIC-16 × HB1 × HB2).
    """
