...
cat > "$OUT/run.ys" <<YEOF
read_verilog -sv $RT/rtl/sd_decimator_hb_w14.v
hierarchy -check -top sd_decimator_hb_w14
synth -top sd_decimator_hb_w14
dfflibmap -liberty $LIB
abc -liberty $LIB
setundef -zero
opt_clean -purge
check
tee -o $OUT/stat_hb_w14.txt stat -liberty $LIB -top sd_decimator_hb_w14
YEOF
yosys -s "$OUT/run.ys" 2>&1 | tee -a "$LOG"

echo "=== Area summary ===" | tee -a "$LOG"
echo "[w14 variant]" | tee -a "$LOG"
grep -E "Number of cells|Chip area" "$OUT/stat_hb_w14.txt" | tee -a "$LOG"
