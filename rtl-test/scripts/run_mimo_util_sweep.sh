#!/bin/bash
# Run one mimo_rx_top LibreLane P&R at a chosen utilization config.
...
exec > >(tee "$LOG") 2>&1
echo "=== mimo_rx_top ${CFG} START $(date --iso-8601=seconds) on $(hostname) ==="
...
# ol_mimo_rx_top/CONGESTION_EXPERIMENT.md.
/foss/tools/bin/librelane --pdk gf180mcuD --scl gf180mcu_fd_sc_mcu7t5v0 \
    --skip Magic.SpiceExtraction \
    ol_mimo_rx_top/${CFG}.json
echo "=== mimo_rx_top ${CFG} EXIT $? $(date --iso-8601=seconds) ==="
