...
//
// CDC (32M -> 16M):
//   strobe_pipe is extended to 2 clk_32m cycles (fir_strobe_ext) so that
...
module sd_decimator_combchain (
    ...
    // Comb stage outputs are now combinational wires (was registered).
    // The 3-stage comb chain runs once per cic_strobe (~1/32..1/256 duty),
    // so the slow datapath has all 32 cycles of slack between strobes for
    // the chained subtractors. Saves 6x 26-bit register banks per instance.
    ...
    // Combinational 3-stage comb chain. comb_*_d are the per-stage delay regs
    // (still required state); the inter-stage *_out values are now wires.
    ...
    always @(posedge clk_32m or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Delay regs advance one position per strobe, fed by the
            // combinational chain outputs (i.e. the value that WAS in *_out
    ...
    // Strobe pipeline: was 3 cycles for the registered comb chain. Now combs
    // collapse to 1 cycle (the shifted_i/q register), so the FIR strobe only
    // needs 1 cycle of delay instead of 3. Kept as a 1-stage register for
    // alignment with shifted_i/q settling.
    reg strobe_pipe;
    always @(posedge clk_32m or negedge rst_n) begin
    ...
    // =========================================================
    // CDC: 32 MHz -> 16 MHz
    // Extend strobe_pipe to 2 clk_32m cycles. This guarantees that
