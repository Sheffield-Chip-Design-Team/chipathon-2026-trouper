...
| `frontend/sc_detector.v` | `models/sync.py:SchmidlCoxDetector` | Aligned at algorithm/configuration level | Updated in this audit for the single selected SC branch (`SC_ANT_SEL`, 0x1B), reset default 0. It is not a cycle-/truncation-exact model of the serial metric engine; those details are RTL/cocotb verified. |
