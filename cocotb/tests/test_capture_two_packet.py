...
async def test_capture_two_packet(dut):
    # `.strip() or default`, NOT .get(name, default): the Makefile exports
    # CAPTURE_NPY, so an unset caller leaves it defined-but-EMPTY and .get()
    # returns "" rather than falling back.
    #
    # The original default (lora_20260619_144822_SF7-BW250-gain30.npy, window
    # 600000/4300000) is absent from the shared mount. This file carries 6
    # bursts ~4.07 M samples apart; the window below spans the first two
    # (564185 and 4630030) with lead-in and tail. That ~4.15 M-sample span is
    # what makes this a ~50 min run, and it is deliberate: the inter-packet
    # gap is not dead air to the DUT. dc_removal's leaky integrator settles
    # across it, psram_buf_ctrl's never-rewinding pointers advance and wrap,
    # and sc_detector's sample_count/timing_ref accumulate -- the last of which
    # is exactly where the post-PSRAM-migration double-count bug hid, because
    # its error grew with elapsed samples. Shortening or splicing the gap would
    # shrink that coverage.
    ...
    start   = _env_int("CAPTURE_START", 562000)
    nsamp   = _env_int("CAPTURE_NSAMP", 4150000)
    ...
    # RX_HOLD (0x1A) is SET out of reset and level-gates sc_clr, so the detector
    # can never lock until firmware releases it -- and this test's whole subject
    # is sc_lock asserting, de-asserting and re-asserting. Release after the
    # locked config writes above (0x09/0x0A/0x0E are only writable while it is
    # held). RX_HOLD is left clear for the rest of the run: re-asserting it
    # between the two packets would force the de-assert this test is trying to
    # observe happen naturally, which would make ARM-1 vacuous.
    # See planning/mcp-config-settle-gate-design.md.
