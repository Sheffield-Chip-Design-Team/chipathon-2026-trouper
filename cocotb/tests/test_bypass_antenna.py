...
async def _check_bypass_ant(dut, antenna_en_mask, expected_bypass_ant):
    ...
    # cannot lock until firmware releases it. Release AFTER the config writes
    # above: 0x09/0x0A/0x0E are in reg_bank's locked set and are only writable
    # while RX_HOLD is held, and ANTENNA_EN (0x08, not locked) must be in place
    # before the sc_lock rising edge that latches active_antenna_en. Holding
    # until here makes that ordering explicit rather than incidental.
