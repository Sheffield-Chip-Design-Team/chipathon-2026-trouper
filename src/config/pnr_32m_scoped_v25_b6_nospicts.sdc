...
# ---- CTS PROBE (Open Risks #6): SPI_SCK is NOT declared a clock in this
# ---- P&R-only SDC, so TritonCTS builds no tree for it and SPI_SCK routes as
# ---- an ordinary net.  Rationale: SPI_SCK has 51 sinks at 2 MHz and needs no
# ---- balanced tree, yet CTS gives it four clkbuf_16 on an NDR root net, and
# ---- the I pin of those buffers is what DRT-1231 cannot reach (jobs 5281/
# ---- 5282/5283).  The SIGNOFF SDC is untouched, so SPI timing is still
# ---- checked at signoff STA -- only the P&R-time tree is suppressed.
# ---- If this routes, the finding is that SPI_SCK should not be a CTS clock.
...
# clock is an error).  Present unchanged in the signoff SDC.
