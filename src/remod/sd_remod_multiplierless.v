...
//
// 4th-order CIFF sigma-delta re-modulator core, instantiated by sd_remod.v.
// PROMOTED TO CANONICAL 2026-09-05 -- see sd_remod.v's header for the full
// rationale and planning/sd-remod-interp2x-pnr-investigation-2026-09-05.md
// for the investigation.
//
// Loop topology, state widths, saturation, coefficients, latency, enable
// behaviour, and I/O interface are bit-exact identical to the pre-promotion
// core (verified: rtl-test/tb/tb_sd_remod_multiplierless_equiv.v, 35,660
// samples, 0 mismatches). Only the Q8 feed-forward cone is rewritten as
// shifts and balanced additions instead of a general Verilog `*` against the
// 10-bit coefficients:
//
//   377*s1 = (3*s1)<<7 - 7*s1
...
//    -8*s3 - 8*s4 = -8*(s3+s4)
//
// This was traced directly to a completed full-chip P&R run's worst SS
// timing path (s1_i -> q_i, 61.25 ns of combinational delay against a
// ~35.5 ns budget) -- a general multiplier against a 10-bit constant
// synthesizes to a considerably deeper/larger structure here than the
// hand-factored network (confirmed: 3,275 -> 2,293 cells, -30% synth area,
// on top of fixing the timing). The complete feed-forward value fits in
// signed 25 bits:
//   (377+106+8+8)*32768 + 254*256 = 16,416,256 < 2^24.
// This lets the final sum and sign test stay at 25 bits instead of the
// original RTL's conservative 27 bits.
